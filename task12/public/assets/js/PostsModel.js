((exports) => {
  function sortByDate(array) {
    ...
    return newArray.sort((a, b) => (new Date(b.createdAt) - new Date(a.createdAt)));
  }
  ...
  exports.editPhotoPost = function editPhotoPost(array, id, photoPost) {
    const oldPhotoPost = findPost(array, id);
  ...
  exports.likePost = function likePost(array, id, author) {
    ...
    const oldPost = findPost(array, id);

    if (oldPost.isDeleted === 'false') {
      const index = oldPost.likes.indexOf(author);
      if (index === -1) {
  ...
  exports.removePhotoPost = function removePhotoPost(id, array) {
    ...
    const post = array.find(x => x.id === id);
